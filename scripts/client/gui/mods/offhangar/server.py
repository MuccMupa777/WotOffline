	...
	def doCmdInt3(self, requestID, cmd, int1, int2, int3):
		...
		if enqueueCmd is not None and cmd == enqueueCmd:
			...
			if player is not None and getattr(player, 'isOffline', False):
				LOG_DEBUG('Offline.FakeServer.ENQUEUE_RANDOM', requestID, int1, int2, int3)
				from gui.mods.offhangar.offline_battle import schedule_random_battle_flow_after_enqueue
				schedule_random_battle_flow_after_enqueue(cmd, 'FakeServer.doCmdInt3', (int1, int2, int3))
